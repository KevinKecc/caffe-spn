...
void ImageLabelmapDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const int new_height = this->layer_param_.image_labelmap_data_param().new_height();
  const int new_width  = this->layer_param_.image_labelmap_data_param().new_width();
  const bool is_color  = this->layer_param_.image_labelmap_data_param().is_color();
  string root_folder = this->layer_param_.image_labelmap_data_param().root_folder();

  CHECK((new_height == 0 && new_width == 0) ||
      (new_height > 0 && new_width > 0)) << "Current implementation requires "
      "new_height and new_width to be set at the same time.";
  // Read the file with filenames and labels
  const string& source = this->layer_param_.image_labelmap_data_param().source();
  ...
  if (this->layer_param_.image_labelmap_data_param().shuffle()) {
  ...
  if (this->layer_param_.image_labelmap_data_param().rand_skip()) {
    unsigned int skip = caffe_rng_rand() %
        this->layer_param_.image_labelmap_data_param().rand_skip();
  ...
  const int batch_size = this->layer_param_.image_labelmap_data_param().batch_size();
...
void ImageLabelmapDataLayer<Dtype>::load_batch(LabelmapBatch<Dtype>* batch) {
  ...
  ImageLabelmapDataParameter image_labelmap_data_param = this->layer_param_.image_labelmap_data_param();
  const int batch_size = image_labelmap_data_param.batch_size();
  const int new_height = image_labelmap_data_param.new_height();
  const int new_width = image_labelmap_data_param.new_width();
  const bool is_color = image_labelmap_data_param.is_color();
  string root_folder = image_labelmap_data_param.root_folder();
